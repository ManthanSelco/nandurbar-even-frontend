...
const exportToExcel = async () => {
  ...
  try {
    ...
    const firstResponse = await api.get("/participants", {
    ...
    const participantsSheet = allParticipants.map(
      (participant: any) => ({
        ...
        "Country Code": participant.countryCode || "",

        "Mobile Verified": participant.mobileVerified
          ? "Yes"
          : "No",
        ...
        "WhatsApp Available":
          participant.whatsappAvailable ? "Yes" : "No",

        "Post Event Step":
          participant.postEventStep || "",

        "Follow Up Required":
          participant.followUpRequired ? "Yes" : "No",

        "Selected Requirement ID":
          participant.selectedRequirement || "",

        "Livelihood Categories":
          Array.isArray(participant.livelihoodCategories)
            ? participant.livelihoodCategories.join(", ")
            : "",

        "Value Chains":
          Array.isArray(participant.valueChains)
            ? participant.valueChains.join(", ")
            : "",

        "Support Solutions":
          Array.isArray(participant.supportSolutions)
        ...
        "Next Actions":
          Array.isArray(participant.nextActions)
            ? participant.nextActions.join(", ")
            : "",

        "Useful At Mela":
          Array.isArray(participant.usefulAtMela)
        ...
        "Recommended Solutions":
          Array.isArray(participant.recommendedSolutions)
        ...
        "Matched Vendor IDs":
          Array.isArray(participant.matchedVendorIds)
            ? participant.matchedVendorIds.join(", ")
            : "",

        "Created At": participant.createdAt || "",
        "Updated At": participant.updatedAt || "",
      })
    );
    ...
    const assessmentsSheet = allParticipants.map(
      (participant: any) => {
        const assessment =
    ...
    allParticipants.forEach((participant: any) => {
      ...
      // Electricity bill is a single document
      if (documents.electricityBill) {
        assessmentDocumentsSheet.push({
          ...
          "File Name":
            documents.electricityBill.fileName || "",

          "File Key":
            documents.electricityBill.fileKey || "",

          "File Type":
            documents.electricityBill.fileType || "",

          "File Size":
            documents.electricityBill.fileSize || "",

          "Uploaded At":
            documents.electricityBill.uploadedAt || "",
        });
      }
    });
    ...
    allParticipants.forEach((participant: any) => {
      ...
      // If no interventions, still keep participant row
    ...
    allParticipants.forEach((participant: any) => {
      const indicators =
        participant.solutionDesign?.indicators || [];
...
>
  ...
  <span
    style={{
      ...
      background:
        implementationStatus === "IMPLEMENTATION_COMPLETED"
          ? "#dcfce7"
          : implementationStatus === "ASSESSMENT_COMPLETED"
            ? "#dcfce7"
            : implementationStatus === "ASSESSMENT_PENDING"
              ? "#fef3c7"
              : implementationStatus === "RE_ASSESSMENT_REQUIRED"
                ? "#fee2e2"
                : implementationStatus === "SOLUTION_PROPOSED"
                  ? "#dbeafe"
                  : implementationStatus === "SOLUTION_APPROVED"
                    ? "#dbeafe"
                    : implementationStatus === "PROCUREMENT"
                      ? "#fef3c7"
                      : implementationStatus === "VENDOR_UPDATE"
                        ? "#e0e7ff"
  ...
  >
    {statusLabel(implementationStatus)}
  </span>
</div>
              </div>
            </td>
